...
import { getAuthorizedConnection } from '../utils/queryRouter';
...
function inferType(val: any): string {
  ...
  if (/^-?\d*\.\d+$/.test(str)) return 'DOUBLE';
  return 'VARCHAR(255)';
}

// Step 1: Analyze dataset (CSV/Excel) and infer schema columns and types
export const analyzeDataset = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (originalName.endsWith('.csv')) {
      ...
      parsedRows = await new Promise<any[]>((resolve, reject) => {
        const rows: any[] = [];
...
export const importDataset = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const userId = req.user?.id;
    ...
    try {
      const existingMeta = await prisma.tableMetadata.findFirst({
      ...
      const colsSql = columns.map((col: any) => `\`${col.logical}\` ${col.type}`).join(', ');
      await connection.query(`CREATE TABLE \`${physicalTableName}\` (${colsSql});`);

      // Map rows to table columns
      const bulkValues = rows.map((row: any) => {
      ...
      const insertHeaders = columns.map((col: any) => `\`${col.logical}\``).join(', ');
      
      const chunkSize = 500;
      for (let i = 0; i < bulkValues.length; i += chunkSize) {
        const chunk = bulkValues.slice(i, i + chunkSize);
        await connection.query(
          `INSERT INTO \`${physicalTableName}\` (${insertHeaders}) VALUES ?`,
          [chunk]
        );
      }
...
export const exportDataset = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
