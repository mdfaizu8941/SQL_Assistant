...
exports.executeSchema = exports.generateSchema = exports.executeQuery = exports.validateQuery = exports.explainQuery = exports.generateQuery = exports.classifyQueryRisk = void 0;
...
const fetchSchemaString = async (userId, role, connectionId, dbConfig) => {
    let connection;
    let databaseName;
 ...
 */
const classifyQueryRisk = async (sqlText) => {
    const dangerous = (0, queryValidator_1.detectDangerousQuery)(sqlText);
    try {
...
Query: ${sqlText}`;
        const content = await (0, aiProvider_1.getAICompletion)([
...
exports.classifyQueryRisk = classifyQueryRisk;
const generateQuery = async (req, res) => {
    try {
        ...
        if (Number(connectionId) !== 9999) {
            dbConfig = await (0, queryRouter_1.getAuthorizedConnection)(Number(connectionId), userId, role);
            if (!dbConfig) {
...
8. **Format**: Respond strictly with JSON. Do not wrap in markdown code block (like \`\`\`json).`;
        ...
        const responseContent = await (0, aiProvider_1.getAICompletion)(messagesPayload);
        let parsed;
        try {
        ...
        if (parsed.sql) {
            risk = await (0, exports.classifyQueryRisk)(parsed.sql);
        }
        res.json({
...
Query: ${sql}`;
        const responseContent = await (0, aiProvider_1.getAICompletion)([
            { role: 'system', content: systemPrompt }
        ]);
...
const validateQuery = async (req, res) => {
    try {
        ...
        const risk = await (0, exports.classifyQueryRisk)(sql);
        res.json({ risk });
    }
    catch (error) {
...
const executeQuery = async (req, res) => {
    try {
        ...
        if (lowerSql === 'show tables' || lowerSql === 'show tables;') {
            const allowedTables = await (0, queryRouter_1.getAllowedTables)(userId, role, false); // Browse-only tables
            const dbUrl = process.env.DATABASE_URL || '';
        ...
        const allowedTables = await (0, queryRouter_1.getAllowedTables)(userId, role, true); // Execution-allowed tables (includes Admin)
        ...
        if (role === 'USER') {
            const isDangerous = (0, queryValidator_1.detectDangerousQuery)(sql).isDangerous;
            if (isDangerous) {
                ...
                const isOwnDatabaseDrop = allowedDbs.some((d) => {
        ...
        const permCheck = (0, queryValidator_1.checkQueryPermissions)(role, sql);
        ...
        const allowedDbs = await (0, queryRouter_1.getAllowedDatabases)(userId, role, true);
        ...
        const physicalTableNames = showTablesResult.map((r) => Object.values(r)[0]);
        ...
        let ddlCreateDbResult = (0, queryRouter_1.rewriteCreateDatabase)(sql, userId, role);
        let ddlDropDbLogicalName = null;
        if (!ddlCreateDbResult) {
            ddlDropDbLogicalName = (0, queryRouter_1.getDropDatabaseLogicalName)(sql, allowedDbs);
        }
        // Rewrite query table names and database namespaces
        let rewrittenSql = sql;
        let ddlCreateResult = null;
        let ddlDropLogicalName = null;
        if (ddlCreateDbResult) {
            rewrittenSql = ddlCreateDbResult.rewrittenSql;
        }
        else {
            // First rewrite USE statement and dbName.tableName qualifiers
            rewrittenSql = (0, queryRouter_1.rewriteDatabaseQueries)(sql, allowedDbs, userId, role);
            ddlCreateResult = (0, queryRouter_1.rewriteCreateTable)(rewrittenSql, userId, role);
            if (ddlCreateResult) {
                rewrittenSql = ddlCreateResult.rewrittenSql;
                rewrittenSql = (0, queryRouter_1.rewriteQuery)(rewrittenSql, activeAllowedTables, userId, role);
            }
            else {
                ddlDropLogicalName = (0, queryRouter_1.getDropTableLogicalName)(rewrittenSql, activeAllowedTables);
                rewrittenSql = (0, queryRouter_1.rewriteQuery)(rewrittenSql, activeAllowedTables, userId, role);
            }
        }
            ...
            if (dryRun) {
                ...
                if (status === 'SUCCESS') {
                    if (ddlCreateDbResult) {
                        ...
                        if (!existing) {
                            await prisma_1.default.databaseMetadata.create({
                                data: {
                                    dbName: logicalName,
                                    physicalName,
                                    ownerId: userId,
                    ...
                    if (ddlCreateResult) {
                        ...
                        if (droppedTable) {
                            await prisma_1.default.tableMetadata.delete({
        ...
        if (!dryRun) {
            ...
            await prisma_1.default.queryHistory.create({
                data: {
                    userId,
...
const executeSchema = async (req, res) => {
    try {
        const userId = req.user?.id;
        ...
        try {
            const allowedDbs = await (0, queryRouter_1.getAllowedDatabases)(userId, role, true);
            const allowedTables = await (0, queryRouter_1.getAllowedTables)(userId, role, false);
            const dynamicDbs = [...allowedDbs];
            const dynamicTables = [...allowedTables];
            for (const statement of statements) {
                let rewrittenStatement = statement;
                // Rewrite USE statements and database qualifiers
                rewrittenStatement = (0, queryRouter_1.rewriteDatabaseQueries)(rewrittenStatement, dynamicDbs, userId, role);
                // Rewrite logical table names to physical table names
                rewrittenStatement = (0, queryRouter_1.rewriteQuery)(rewrittenStatement, dynamicTables, userId, role);
                // Check if database create DDL
                const createDbResult = (0, queryRouter_1.rewriteCreateDatabase)(rewrittenStatement, userId, role);
                if (createDbResult) {
                    ...
                    const createTableResult = (0, queryRouter_1.rewriteCreateTable)(rewrittenStatement, userId, role);
...
exports.executeSchema = executeSchema;
