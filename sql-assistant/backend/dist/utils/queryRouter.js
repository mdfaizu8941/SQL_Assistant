...
exports.getAuthorizedConnection = getAuthorizedConnection;
exports.getAllowedTables = getAllowedTables;
exports.resolveTable = resolveTable;
exports.getAllowedDatabases = getAllowedDatabases;
exports.resolveDatabase = resolveDatabase;
exports.rewriteDatabaseQueries = rewriteDatabaseQueries;
exports.rewriteQuery = rewriteQuery;
exports.rewriteCreateTable = rewriteCreateTable;
exports.getDropTableLogicalName = getDropTableLogicalName;
exports.rewriteCreateDatabase = rewriteCreateDatabase;
exports.getDropDatabaseLogicalName = getDropDatabaseLogicalName;
const prisma_1 = __importDefault(require("./prisma"));
/**
 * Resolves whether a connection configuration is authorized for a specific user role.
 */
async function getAuthorizedConnection(connectionId, userId, role) {
    ...
    if (Number(connectionId) >= 100000) {
        ...
        const dbMeta = await prisma_1.default.databaseMetadata.findFirst({
    ...
    return await prisma_1.default.databaseConnection.findFirst({
 ...
 */
async function getAllowedTables(userId, role, forExecution = false) {
    // USER - Own tables OR SHARED/PUBLIC tables
    return await prisma_1.default.tableMetadata.findMany({
        where: {
            OR: [
                { ownerId: userId },
                { visibility: 'SHARED' },
 ...
 */
function resolveTable(logicalName, allowedTables, userId, role) {
    const matches = allowedTables.filter((t) => t.tableName.toLowerCase() === logicalName.toLowerCase());
    if (matches.length === 0)
        return null;
    ...
    const getPriority = (table) => {
        ...
        if (isOwn)
            return 1;
        if (table.visibility === 'SHARED' || table.visibility === 'PUBLIC')
            return 2;
        return 3;
    };
 ...
 */
async function getAllowedDatabases(userId, role, forExecution = false) {
    // USER - Own databases OR SHARED/PUBLIC databases
    return await prisma_1.default.databaseMetadata.findMany({
        where: {
            OR: [
                { ownerId: userId },
                { visibility: 'SHARED' },
 ...
 */
function resolveDatabase(logicalDbName, allowedDbs, userId, role) {
    const matches = allowedDbs.filter((d) => d.dbName.toLowerCase() === logicalDbName.toLowerCase());
    if (matches.length === 0)
        return null;
    // Security isolation filter
    ...
    const getPriority = (db) => {
        ...
        if (isOwn)
            return 1;
        if (db.visibility === 'SHARED' || db.visibility === 'PUBLIC')
            return 2;
        return 3;
    };
    allowedMatches.sort((a, b) => getPriority(a) - getPriority(b));
 ...
 */
function cleanAndExtractLiterals(sql) {
    const literals = [];
 ...
 */
function rewriteDatabaseQueries(sql, allowedDbs, userId, role) {
 ...
 */
function rewriteQuery(sql, allowedTables, userId, role) {
 ...
 */
function rewriteCreateTable(sql, userId, role) {
 ...
 */
function getDropTableLogicalName(sql, allowedTables) {
 ...
 */
function rewriteCreateDatabase(sql, userId, role) {
    const normalized = sql.replace(/\s+/g, ' ');
 ...
 */
function getDropDatabaseLogicalName(sql, allowedDbs) {
    const normalized = sql.replace(/\s+/g, ' ').toLowerCase();
